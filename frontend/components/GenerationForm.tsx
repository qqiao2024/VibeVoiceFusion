...
export default function GenerationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Always send batch_size (single generation is just batch_size=1)
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="seeds" className="block text-sm font-medium text-gray-700 mb-1">
            {t('generation.initialSeed')} <span className="text-red-500">*</span>
          </label>
          ...
          <p className="text-xs text-gray-500 mt-1">
            {t('generation.initialSeedDescription')}
          </p>
        </div>
