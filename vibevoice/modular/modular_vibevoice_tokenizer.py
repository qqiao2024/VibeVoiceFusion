...
class ConvLayerNorm(nn.LayerNorm):
    ...
    def forward(self, x):
        ...
        x = nn.functional.layer_norm(x.float(),
...
class Convlayer(nn.Module):
    def __init__(self,
                 ...
                 causal=True):
        super().__init__()
        self.conv = SConv1d(in_channels, out_channels, kernel_size,
                            stride=stride, dilation=dilation, groups=groups,
                            bias=bias, pad_mode=pad_mode, norm=norm, causal=causal)
...
class TokenizerEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        stem = nn.Sequential(SConv1d(self.channels,
                                     self.n_filters,
                                     kernel_size,
                                     norm=norm,
...
class TokenizerDecoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        stem = nn.Sequential(SConv1d(self.dimension,
