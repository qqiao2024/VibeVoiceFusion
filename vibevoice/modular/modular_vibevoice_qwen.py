...
class QwenRotaryEmbedding(nn.Module):
    def __init__(self, config: QwenConfig, device=None):
        ...
        self.config = config
        self.rope_init_fn = default_rope_parameters
...
class QwenModel(nn.Module):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`QWenLayerDecoder`]

    Args:
        config: QWenConfig
    """

    def __init__(self, config: QwenConfig, dtype: torch.dtype = torch.bfloat16):
        super().__init__()
    ...
    def forward(
        ...
        **kwargs,
        ...
        # causal_mask = self._update_causal_mask(attention_mask, inputs_embeds, cache_position, output_attentions)
        causal_mask = None
